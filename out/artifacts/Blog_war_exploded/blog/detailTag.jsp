...
<%
    ...
    request.setAttribute("ShowTagDetail",selAllBlogToHtml(blogService.selBlogByTagName(tagName)));
    ...
    pageBreak.setTotal(blogService.selBlogByTagName(tagName).size());
...
<div id="body-wrap">
    ...
    <main class="layout_page" id="content-inner">
        <div id="archive">
            <div class="article-sort-title">标签 - <%=tagName%> </div>
            <div class="article-sort">
                <div class="article-sort-item year" id="getFullYear">2020</div>

                <c:forEach items="${requestScope.ShowTagDetail}" var="blog" begin="0" end="${pageBreak.count}" step="1" >
            ...
            <nav id="pagination">
                <div class="pagination">
                    <a class="extend prev" id="prevBlogPage" rel="prev" href="blog/detailTag.jsp?page=${pageBreak.pageNumber-1}">
                        <i class="fa fa-fw fa-chevron-left" aria-hidden="true"></i>
                    </a>
                    <a id="FirstPage" class="page-number current" href="blog/detailTag.jsp?page=1">1</a>
                    <c:forEach begin="2" end="3" step="1" varStatus="var">
                        <a class="page-number" href="blog/detailTag.jsp?page=${var.current}">${var.current}</a>
                    </c:forEach>
                    ...
                    <c:if test="${pageBreak.pageNumber>3}">
                        ...
                        <c:if test="${pageBreak.pageNumber!=4}" >
                            <a class="page-number" href="blog/detailTag.jsp?page=${pageBreak.pageNumber-1}">${pageBreak.pageNumber-1}</a>
                        </c:if>

                        <c:if test="${pageBreak.getTotalPage()!=pageBreak.pageNumber}">
                            <a class="page-number" href="blog/detailTag.jsp?page=${pageBreak.pageNumber}">${pageBreak.pageNumber}</a>
                        </c:if>
                    ...
                    <c:if test="${pageBreak.getTotalPage()>3}">
                        <a class="page-number" href="blog/detailTag.jsp?page=${pageBreak.getTotalPage()}">${pageBreak.getTotalPage()}</a>
                    </c:if>

                    <a class="extend next" id="nextBlogPage" rel="next" href="blog/detailTag.jsp?page=${pageBreak.pageNumber+1}">
...
<script>
    $(function () {
        ...
        function goToPage() {
            ...
            window.location.href="blog/detailTag.jsp?page="+pageNum;
        }
